...
class Sanksi_api extends MY_Controller
{
    ...
    // Variables
    private $datatablesEndpoint = 'sanksi_datatables';
    private $createEndpoint = 'tambah_sanksi';
    private $updateEndpoint = 'edit_sanksi';
    private $deleteEndpoint = 'hapus_sanksi';
    ...
    public function index()
    {
        ...
        // Main
        $this->load->view('data_master/sanksi/view');

        // Modal
        $this->load->view('components/modal/sanksi');

        // JS
        $this->load->view('components/js');

        // Page JS
        $this->load->view('components/page_js/sanksi/view');
    ...
    public function tambah_sanksi()
    {
        ...
        // Main
        $this->load->view('data_master/sanksi/add');

        // JS
        $this->load->view('components/js');

        // Page JS
        $this->load->view('components/page_js/sanksi/add');
    ...
    {
        $this->session->unset_userdata('id_sanksi');
        $auth_sanksi = htmlspecialchars(trim($this->input->post("auth")));
        $tokenAuth = $this->session->userdata('token');
        ...
        if ($result['status'] == 200) {
            if ($result['data'][0]['stat_sanksi'] == "T") {
                $status = "AKTIF";
            } else {
                $status = "NONAKTIF";
            }
            ...
            $data = [
                'statusCode' => 200,
                'kode' => $result['data'][0]['kd_sanksi'],
                'sanksi' => $result['data'][0]['sanksi'],
                'durasi' => $result['data'][0]['jml_hari_berlaku'],
                'keterangan' => $result['data'][0]['ket_sanksi'],
                'status' => $status,
                'tgl_buat' => date('d-M-Y H:i:s', strtotime($result['data'][0]['tgl_buat'])),
                'pembuat' => $result['data'][0]['nama_user'],
            ];

            $this->session->set_userdata('id_sanksi', $result['data'][0]['id_sanksi']);
            echo json_encode($data);
        } else {
            echo json_encode(array('statusCode' => 404, "kode_pesan" => "Gagal", "pesan" => "Sanksi tidak ditemukan!", "tipe_pesan" => "error"));
        }
    }
    ...
    public function insert()
    {
        ...
        $sanksi = htmlspecialchars(trim($this->input->post("sanksi", true)));
        $durasi = htmlspecialchars(trim($this->input->post("durasi", true)));
        ...
        if ($checkData['status'] == 200) {
            echo json_encode(array("statusCode" => 400, "kode_pesan" => "Gagal", "pesan" => "Sanksi sudah ada!", "tipe_pesan" => "error"));
            return;
        }
        ...
        if ($result == 201) {
            echo json_encode(array("statusCode" => 200, "kode_pesan" => "Berhasil", "pesan" => "Sanksi berhasil disimpan", "tipe_pesan" => "success"));
        } else {
            echo json_encode(array("statusCode" => 400, "kode_pesan" => "Gagal", "pesan" => "Sanksi gagal disimpan", "tipe_pesan" => "error"));
        }
    }

    public function update()
    {
        $id = $this->session->userdata('id_sanksi');
        ...
        if ($checkData['status'] == 200) {
            echo json_encode(array("statusCode" => 201, "kode_pesan" => "Gagal", "pesan" => 'Sanksi sudah ada!', "tipe_pesan" => "error"));
            return;
        }
        ...
        if ($result == 200) {
            $this->session->unset_userdata('id_sanksi');
            echo json_encode(array("statusCode" => 200, "kode_pesan" => "Berhasil", "pesan" => "Sanksi berhasil diupdate!", "tipe_pesan" => "success"));
        } else {
            echo json_encode(array("statusCode" => 201, "kode_pesan" => "Gagal", "pesan" => 'Sanksi gagal diupdate!', "tipe_pesan" => "error"));
        }
    }
    ...
    public function delete()
    {
        ...
        if ($result == 200) {
            echo json_encode(array("statusCode" => 200, "kode_pesan" => "Berhasil", "pesan" => "Sanksi berhasil dihapus!", "tipe_pesan" => "success"));
        } else {
            echo json_encode(array("statusCode" => 201, "kode_pesan" => "Gagal", "pesan" => "Sanksi gagal dihapus!", "tipe_pesan" => "error"));
        }
    }
}